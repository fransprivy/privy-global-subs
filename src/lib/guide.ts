...
export const GUIDES: Record<string, Guide> = {
  free: {
    ...
    steps: [
      ...
      {
        id: "free-5",
        title: "Buy Business and get a second workspace",
        do: "Reset scenario. Plans → Upgrade to Business → 3 seats → name the workspace → card 4242 → consent → Pay.",
        expect: "You land in the new Business workspace with a welcome step (M-15). Open the avatar menu: two workspaces. Switch to your Individual one: the plan reads 'Personal · included with Business' with unlimited envelopes and no charge.",
        href: "/plans",
        refs: ["M-15", "R-73", "R-74", "R-76"],
        note: "Reset the scenario first.",
        check: (s) => has(s, "workspace_created"),
      },
    ],
  },
  ...
  "business-owner": {
    ...
    goal: "Prove the two-workspace model for an owner (perk in Individual, team in Business), then seats prorated to the one renewal date, backup cards, and the downgrade checklist.",
    steps: [
      {
        id: "bo-ws-1",
        title: "Switch between your workspaces",
        do: "Avatar menu (top right) → 'Frans · Individual'. Look at the plan chip on Home and Settings → Billing. Then switch back to 'Privy Product Team'.",
        expect: "Individual shows 'Personal · included with Business' with unlimited envelopes and a billing card that points to the Business workspace. Business shows 'Business · active' with seats, members, invoices.",
        href: "/home",
        refs: ["R-73", "R-76", "R-77"],
        check: (s) => s.activeWorkspace === "individual",
      },
      {
        id: "bo-ws-2",
        title: "Invite a member into a free seat",
        do: "Business workspace → Billing → Team members → Invite member → any email → Send.",
        expect: "6 of 8 seats were used, so the invite works without buying seats. The list grows to 7; the invitee's own plan is untouched (R-71). Try again after all 8 seats are used: the button explains you need more seats.",
        href: "/settings/billing",
        refs: ["R-78", "R-71"],
        check: (s) => has(s, "member_invited"),
      },
      {
        id: "bo-ws-2b",
        title: "Make an admin, and move envelopes into the Business workspace",
        do: "Team members → 'Make admin' next to Rima. Then switch to Individual → Envelopes → select → 'Hand over' → Privy Product Team.",
        expect: "Rima's chip turns to Admin (admins manage members, seats and cards; only you change the plan, transfer, set admins, delete). The envelopes appear in the Business workspace list.",
        href: "/settings/billing",
        refs: ["R-82", "R-79"],
        check: (s) => has(s, "role_changed"),
      },
      {
        id: "bo-ws-3",
        title: "Invite when every seat is taken: seat added in the same flow",
        do: "Invite two more members so 8 of 8 seats are used, then click 'Invite member (+1 seat)' again → email → tick the consent → 'Add seat, pay A$26.95 and invite'.",
        expect: "One modal does both: prorated charge for 1 seat (21 of 30 days, same Oct 1 renewal), then the invitation. Seats become 9, invoice and email N-22 for the seat, history shows both.",
        href: "/settings/billing",
        refs: ["UX-29", "M-09", "R-12"],
        check: (s) => (s.subscription?.seats ?? 0) >= 9 && has(s, "member_invited"),
      },
      {
        id: "bo-ws-4",
        title: "Transfer ownership to a member",
        do: "Team members → 'Make owner' next to Kenny Hartono → read the billing consequences → tick → Transfer.",
        expect: "Your card is detached (subscription ended, no further charges). The workspace moves to 'Other workspaces' with Kenny as owner and banner B-10 'has not set up payment yet' until Oct 1. Your Individual workspace is back to Free. Emails N-35 (to Kenny) and N-36 (to you).",
        refs: ["M-18", "R-81", "N-35", "N-36", "B-10"],
        note: "Do this last, or reset afterwards: the other steps need you to be the owner.",
        check: (s) => has(s, "ownership_transferred"),
      },
      {
        id: "bo-seats-1",
      ...
      },
      {
        ...
        expect: "A$7.49 charged for Personal Monthly. Privy Product Team becomes read-only (view and download only) for you and the 5 members, and stays in the switcher marked Expired. Emails N-12 to Frans and N-16 to each member.",
  ...
  "business-member-free": {
    scenarioId: "business-member-free",
    goal: "Membership never upgrades your own plan: Free stays Free with 5 envelopes, while the owner's workspace is unlimited.",
    steps: [
      {
        id: "bmf-1",
        title: "Compare the two workspaces",
        do: "Home shows 'Frans · Individual · Free' (4 sends left). Avatar menu → 'Hartono Legal Partners'.",
        expect: "Kenny's Business workspace: unlimited envelopes, its own task list, no Upgrade button in the top bar. Billing says 'managed by Kenny Hartono' with no actions.",
        href: "/home",
        refs: ["R-71", "R-76"],
        check: (s) => s.activeWorkspace === "ws_kenny",
      },
      {
        id: "bmf-2",
        title: "Plans page inside a member workspace",
        do: "In Kenny's workspace open Upgrade plan (or Settings → Billing → Change plan).",
        expect: "No plan cards: 'Plans are chosen per workspace' with a button back to your Individual workspace.",
        href: "/plans",
        refs: ["R-71"],
      ...
      },
      {
        ...
        expect: "A third workspace appears: yours. Your Individual workspace becomes 'Personal · included with Business' (unlimited). Kenny's workspace is unchanged.",
        href: "/plans",
        refs: ["R-70", "R-73", "R-74", "M-15"],
        check: (s) => has(s, "workspace_created"),
      },
    ],
  },
  "business-member-personal": {
    scenarioId: "business-member-personal",
    goal: "A paying Personal subscriber who is also a member: the 50-envelope limit stays until they buy Business themselves.",
    steps: [
      {
        id: "bmp-1",
        title: "Check the limit in the Individual workspace",
        do: "Home / Billing in 'Frans · Individual'.",
        expect: "Personal Monthly, 47 of 50 envelopes used, 3 sends left. Membership of Kenny's Business and PT Privy (Demo) Enterprise does not lift this.",
        href: "/settings/billing",
        refs: ["R-71"],
      ...
      {
        id: "bmp-ending",
        title: "Owner cancelled: members are warned; you are an admin there",
        do: "Avatar menu → Hartono Legal Partners → Billing.",
        expect: "Banner B-10: Kenny's plan ends on Oct 1, 2026 and the workspace becomes read-only then. Billing shows 'You are an admin': you can invite and remove members (5 of 6 seats), but the plan, seats and cards stay with Kenny.",
        refs: ["B-10", "R-82"],
        check: (s) => s.activeWorkspace === "ws_kenny",
      },
      {
        id: "bmp-handover",
        title: "Take your own envelope with you",
        do: "Envelopes in Kenny's workspace → only 'Consultation notes' can be ticked (you uploaded it) → Hand over → Frans (Individual).",
        expect: "It moves to your Individual workspace; Kenny's envelopes cannot be selected.",
        refs: ["R-79"],
        check: (s) => has(s, "handover"),
      },
      {
        id: "bmp-2",
        title: "Look at the Enterprise workspace",
        do: "Avatar menu → PT Privy (Demo).",
        expect: "Enterprise · active, contract until Mar 31, 2027, 'Contact sales', no self-serve plan. Unlimited envelopes there.",
        refs: ["R-71"],
        check: (s) => s.activeWorkspace === "ws_ent_demo",
      },
      {
        id: "bmp-3",
        title: "Upgrade Personal to Business (rule C still applies)",
        do: "Individual → Change plan → Upgrade to Business → choose 'Upgrade now' → acknowledge the forfeited Personal days → name the workspace → Pay.",
        expect: "Personal ends today (no credit), Business starts with a new billing date, your own Business workspace is created and Individual becomes 'included with Business'.",
        href: "/settings/billing/change-plan",
        refs: ["Rule C", "UX-03", "R-74"],
        check: (s) => has(s, "workspace_created"),
      },
    ],
  },
  "business-expired": {
    scenarioId: "business-expired",
    goal: "An expired Business workspace is read-only, not gone: prove view/download only, the handover to Individual, and reactivation.",
    steps: [
      {
        id: "bx-1",
        title: "Read-only workspace",
        do: "Home and Envelopes in 'Privy Product Team'.",
        expect: "Banner B-09 'read-only since Sep 1, 2026'. Send an envelope and Sign are disabled; Download works. Top bar says 'Reactivate plan'. The switcher marks the workspace Expired.",
        href: "/envelopes",
        refs: ["R-72", "B-09"],
        check: (_s, p) => p === "/envelopes",
      },
      {
        id: "bx-2",
        title: "Individual is back to Free",
        do: "Avatar menu → 'Frans · Individual'.",
        expect: "Plan chip 'Free', 5 envelopes a month. The owner perk ended with the Business plan.",
        refs: ["R-73"],
        check: (s) => s.activeWorkspace === "individual",
      },
      {
        id: "bx-3",
        title: "Hand envelopes over, to you or to each uploader",
        do: "Privy Product Team → Envelopes → select all → 'Hand over 4' → choose 'Frans (Individual)' or 'Each uploader's Individual workspace' → confirm.",
        expect: "To you: all 4 land in your Individual workspace. To each uploader: Kenny's and Rima's go to them, yours to you. The Business workspace is left empty with the handover date.",
        refs: ["R-79", "UX-08"],
        check: (s) => has(s, "handover"),
      },
      {
        ...
        do: "Billing → 'Delete workspace' → keep 'Move envelopes to my Individual workspace' ticked → type the name → Delete.",
        expect: "The workspace disappears from the switcher. The plan page offers 'Upgrade to Business' again, which would create a brand-new workspace.",
        refs: ["M-19", "R-83"],
        note: "Optional. Reset the scenario before step 4.",
        check: (s) => has(s, "workspace_deleted"),
      },
      {
        id: "bx-4",
        title: "Reactivate Business",
        do: "Billing → 'Reactivate Business' → 4 seats → card 4242 → consent → Pay.",
        expect: "New subscription from today (new billing date), workspace active again, members can sign, Individual returns to 'included with Business'.",
        href: "/settings/billing",
        refs: ["R-75"],
        check: (s) => has(s, "workspace_reactivated"),
      },
    ],
  },
  "enterprise-member": {
    scenarioId: "enterprise-member",
    goal: "The production switcher: Individual plus Enterprise workspaces, one of them expired. Enterprise has no self-serve billing.",
    steps: [
      {
        id: "em-1",
        title: "Open the switcher",
        do: "Avatar menu.",
        expect: "Current: Frans · Individual. Others: PT Privy (Demo) · Enterprise, PT Privy Demo May 2026 · Enterprise · Expired. Settings, Help centre, Log out below.",
        href: "/home",
        refs: ["R-76"],
      },
      {
        id: "em-2",
        title: "Expired Enterprise workspace",
        do: "Switch to PT Privy Demo May 2026 → Envelopes.",
        expect: "Read-only banner with a contact-sales note; Download works, nothing else. Billing shows the contract ended Aug 31, 2026.",
        refs: ["R-72"],
        check: (s) => s.activeWorkspace === "ws_ent_may",
      },
      {
        id: "em-3",
        title: "Your own plan is still yours",
        do: "Switch to Individual → Upgrade plan.",
        expect: "Free with the normal plan cards. Buying Personal or Business here never touches the Enterprise workspaces.",
        href: "/plans",
        refs: ["R-71"],
        check: (s, p) => s.activeWorkspace === "individual" && p.startsWith("/plans"),
      },
    ],
  },
  "id-free": {
