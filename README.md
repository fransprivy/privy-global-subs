...
- **Paywall (M-17)**: Send / New envelope opens the production "Upload a document" screen; choosing a file counts one envelope against the workspace quota (UX-28). With 0 sends left the paywall opens on top of it, styled after production ("Your plan does not go this far"): illustration, the numbers and reset date, Personal and Business options (Business only for Personal users), Not now / Upgrade plan. The drop zone stays blocked until the plan is upgraded or the counter resets.
- **Seat exhausted on invite (UX-29)**: when all seats are used, the invite modal adds one seat, charges it prorated to the renewal date and sends the invitation in one step.
- **Reactivation (R-75)**: once a Business workspace has expired, Business is offered as "Reactivate Business" everywhere (plans page in any workspace, billing, checkout title), never as "Upgrade to Business".
- **Handover (R-79)**: select envelopes on the Envelopes page and hand them over: Individual → own Business workspace, own Business → your Individual workspace or to each uploader's Individual workspace, member workspace → your Individual workspace (own uploads only). Works in expired workspaces too.
- **Delete workspace (M-19, R-83)**: owner of an expired workspace can delete it (optionally moving envelopes to Individual first, name confirmation); the next Business purchase creates a new workspace.
- **Roles (R-82)**: owner sets admins. Admins invite and remove members, change seats and manage payment methods. Only the owner can change or cancel the plan, transfer ownership, set admins, hand over everything or delete the workspace. Scenario "Personal subscriber who is also a member" shows the admin view in Kenny's workspace.
- **Prototype helpers** (requirement tags, test-card hints, support panel, explanatory notes) are hidden unless "Show prototype helpers" is on in the Prototype panel, so screens read as production copy.
- **Ownership transfer (M-18, R-81)**: Team members → Make owner. The old owner's card is detached at once (subscription ended, nothing charged again, no refund), the paid period keeps the workspace active, and the new owner must add a payment method before it ends or the workspace becomes read-only. The old owner loses the Individual perk immediately and stays as a member; emails N-35 (new owner) and N-36 (old owner).
- **Member warnings (B-10)**: members of a workspace whose owner's plan is ending, or whose new owner has not set up payment, see a banner with the date. Indonesian one-time bills for Business mention how many members would lose signing access (banner B-06, emails N-30, N-30b, N-31).
